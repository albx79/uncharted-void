...
#let affinity-pill(attr, color) = box(
  fill: color, radius: 2pt, inset: (x: 2pt, y: 1pt),
...
#let attr-strip(attrs, highlight) = grid(
  ...
  gutter: 0mm,
  ..attrs.keys().enumerate(start: 1).map(x => {
    let attr = x.at(1)
    let i    = x.at(0)
    attr-section(
      attr, attrs.at(attr),
...
#let card-frame(cs) = place(top + left, dx: 2mm, dy: 2mm)[
  #polygon(
    (0mm, 0mm), (59.5mm, 0mm),
    (59.5mm, 84.9mm - cs), (59.5mm - cs, 84.9mm),
    (0mm, 84.9mm),
    stroke: 1pt + frame-col, fill: none,
  )
]
...
#let card-art(art) = place(top + left,
  if art != none {
    ...
    #place(top + left, dx: 2mm, dy: 2mm)[
      #block(width: 59.5mm, height: 84.9mm, fill: none)[
        ...
        #place(bottom + left, dx: 2mm, dy: -2mm)[
          #block(
            clip: true, width: 55.5mm, fill: text-bg,
            radius: r, stroke: 0.75pt + frame-col, inset: 0mm,
          )[
    ...
    #place(top + left, dx: 2mm, dy: 2mm)[
      #block(width: 59.5mm, height: 84.9mm, fill: none)[
        ...
        #place(bottom + left, dx: 9mm, dy: -2mm)[
          #rotate(90deg, reflow: true, block(
            clip: true, width: 80.9mm, fill: text-bg,
            radius: r, stroke: 0.75pt + frame-col, inset: 0mm,
          )[
...
#let draw-card(row) = {
  ...
  if card-type == "Location" {
    draw-location(
      name: name, card-type: card-type,
      subtypes: subtypes, attrs: attrs, highlight: highlight,
      rules: rules, tracker: tracker, rarity: rarity, art: art,
    )
  } else if card-type == "Ship" {
    draw-landscape(
      name: name, card-type: card-type, cost: cost,
      affinity: affinity, subtypes: subtypes, attrs: attrs,
      highlight: highlight, rules: rules, rarity: rarity, art: art,
    )
  } else {
    draw-portrait(
      name: name, card-type: card-type, cost: cost,
...
// ── Sample renders ────────────────────────────────────────────────────────────
#draw-portrait(
  name: "Smaragdine Archon", card-type: "Sophont", cost: 4,
  affinity: ("WRD", "CHA"),
...
#draw-landscape(
  name: "Eidolon", card-type: "Ship", cost: 4,
  affinity: ("WRD", "STR"),
...
#draw-location(
  name: "Mira's Nebula", card-type: "Location",
